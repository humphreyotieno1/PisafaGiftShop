...
export default function CartContent() {
  ...
  const handleCheckout = async () => {
    ...
    const phone = window.prompt('Enter M-Pesa phone number:') || ''
    if (!address || !phone) return

    setIsProcessing(true)
    try {
      const res = await checkout({ payment_method: 'mpesa', address, phone_number: phone })
      if (res) {
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <motion.div key={item.product_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="grid grid-cols-1 border-b p-4 sm:grid-cols-6">
                <div className="col-span-3 flex items-center">
                  ...
                  <div className="ml-4 flex flex-1 flex-col">
                    ...
                    <button onClick={() => handleRemoveItem(item.product_id)} className="mt-1 flex w-fit items-center text-xs text-destructive hover:underline sm:hidden">
                ...
                <div className="col-span-1 flex items-center justify-between sm:justify-center">
                  ...
                  <div className="flex items-center">
                    <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleQuantityChange(item.product_id, Math.max(0, item.quantity - 1))}><Minus className="h-3 w-3" /></Button>
                    <span className="mx-2 w-8 text-center">{item.quantity}</span>
                    <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleQuantityChange(item.product_id, item.quantity + 1)}><Plus className="h-3 w-3" /></Button>
                  </div>
                </div>
                ...
                <button onClick={() => handleRemoveItem(item.product_id)} className="col-span-6 mt-2 hidden items-center justify-end text-xs text-destructive hover:underline sm:flex">
                  <Trash2 className="mr-1 h-3 w-3" /> Remove
                </button>
              </motion.div>
            ))}
        ...
        <div className="lg:col-span-1">
          <div className="rounded-lg border p-6">
            ...
            <Button onClick={handleCheckout} className="mt-6 w-full gap-2" size="lg" disabled={isProcessing}>{isProcessing ? "Processing..." : "Checkout"}{!isProcessing && <ArrowRight className="h-4 w-4" />}</Button>
            <div className="mt-4 text-center"><Link href="/shop" className="text-sm text-muted-foreground hover:text-foreground hover:underline">Continue Shopping</Link></div>
          </div>
        </div>
      </div>
    </div>
